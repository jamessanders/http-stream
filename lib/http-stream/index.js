...
function HttpStream (id, request, response) {
...
HttpStream.prototype.changeReadyState = function (ready) {
  this.updateAge();
  this.is_ready = ready;
}
    ...
    function unmark (event) {
      return function () {
        if (req == self.request) {
          ...
          console.log("Req is req: " + (req == self.request));
          ...
          req.connection.removeAllListeners("end");
        }
      }
    }
