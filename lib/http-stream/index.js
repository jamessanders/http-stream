...
function HttpStream (id) {
  ...
  this.delimiter = "9da623576cdb43b3995be6b09c3f235d";

}

HttpStream.prototype = events.EventEmitter.prototype;

HttpStream.prototype.setMultipart = function (b) {
  this.multipart = b;
};
...
HttpStream.prototype.updateResponseObject = function (req, res) {
  ...
  if (this.multipart) {
    res.connection.write("HTTP/1.1 200\r\nAccess-Control-Allow-Origin: *\r\nConnection: keep-alive\r\nContent-type: multipart/x-mixed-replace; boundary="+this.delimiter+"\r\n\r\n");
    res.connection.write("--"+this.delimiter);
  }
...
HttpStream.prototype.tick = function () {
  ...
  if (this.isReady() && this.wait_buffer.length > 0 && (connection = this.getNextConnection())) {
    ...
    if (!this.multipart){
      ...
      response.connection.write("Content-Type: application/octet-stream\r\n\r\n");
    }
    while (this.wait_buffer.length > 0) {
      ...
      if (!this.multipart) {
        ...
        response.connection.write(packet[0]);
        response.connection.write('\x00');
      }
    ...
    if (!this.multipart) {
      ...
      response.connection.write("--"+this.delimiter);
  ...
  httpServer.on("request", function (req, res) {
    ...
    if (req.url.match(new RegExp("^/"+prefix+"/open$"))) {
      ...
      if (session_id && streams[session_id]) {
        ...
        if (req.headers["accept"] == 'multipart/x-mixed-replace') {
          streams[session_id].setMultipart(true);
        }
